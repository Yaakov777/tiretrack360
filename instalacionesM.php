<div class="posicion-badge bg-<?= 
                                                        $instalacion['posicion'] <= 2 ? 'primary' : 
                                                        ($instalacion['posicion'] <= 4 ? 'info' : 'secondary') 
                                                    ?>">
...
<td>
    ...
    <small class="text-muted"><?= htmlspecialchars($instalacion['equipo_nombre']) ?></small>
</td>
<td>
    <strong><?= htmlspecialchars($instalacion['codigo_interno']) ?></strong><br>
    <small class="text-muted"><?= htmlspecialchars($instalacion['numero_serie']) ?></small>
</td>
<td>
    <strong><?= htmlspecialchars($instalacion['marca_nombre']) ?></strong><br>
    <small class="text-muted"><?= htmlspecialchars($instalacion['diseno_nombre']) ?></small>
    <span class="badge bg-light text-dark"><?= htmlspecialchars($instalacion['medida_nombre']) ?></span>
</td>
<td>
    <div class="text-center">
        <div class="fw-bold"><?= number_format($instalacion['cocada_actual'], 1) ?></div>
        <small class="text-muted">mm</small>
        <?php if ($instalacion['cocada_inicial'] != $instalacion['cocada_actual']): ?>
        <br><small class="text-info">Inicial: <?= number_format($instalacion['cocada_inicial'], 1) ?></small>
        <?php endif; ?>
    </div>
</td>
<td>
    ...
    <div class="progress desgaste-progress">
        <div class="progress-bar bg-<?= $color ?>" style="width: <?= min($desgaste, 100) ?>%">
...
<td class="text-center">
    ...
    <br><small class="text-info"><?= formatDate($instalacion['fecha_instalacion']) ?></small>
</td>
<td class="text-center">
    <div class="fw-bold"><?= number_format($instalacion['horas_acumuladas']) ?></div>
    ...
    <br><small class="text-success">S/<?= number_format($costo_hora, 2) ?>/hr</small>
    <?php endif; ?>
</td>
<td class="text-center">
...
<td class="text-center">
    <div class="btn-group btn-group-sm">
        ...
        <button type="button" class="btn btn-outline-info btn-action" onclick="verHistorial(<?= $instalacion['id'] ?>)"
            title="Ver historial">
    ...
    <nav aria-label="Paginación de instalaciones">
        <ul class="pagination pagination-sm mb-0 justify-content-center">
            <li class="page-item <?= $page <= 1 ? 'disabled' : '' ?>">
                <a class="page-link"
                    href="?page=<?= $page - 1 ?>&<?= http_build_query(array_filter($_GET, function($k) { return $k != 'page'; }, ARRAY_FILTER_USE_KEY)) ?>">
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            ...
                            <select class="form-select" id="equipo_id" name="equipo_id" required
                                ...
                                <option value="<?= $equipo['id'] ?>"><?= $equipo['codigo'] ?> - <?= $equipo['nombre'] ?>
                                </option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                        <div class="col-md-6 mb-3">
                            ...
                            <div class="form-text">Las posiciones disponibles se cargarán al seleccionar el equipo</div>
                        </div>
                    </div>
                    ...
                    <div class="row">
                        <div class="col-12 mb-3">
                            ...
                            <select class="form-select" id="neumatico_id" name="neumatico_id" required
                                ...
                                <option value="<?= $neumatico['id'] ?>" data-costo="<?= $neumatico['costo_nuevo'] ?>"
                ...
                <div class="modal-body">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <select class="form-select" id="rotacion_destino_select" onchange="seleccionarDestino()">
            ...
            <form method="POST" id="retirarForm">
                ...
                <div class="modal-body">
                    <div class="alert alert-warning">
                        ...
                        <p class="mb-0">Va a retirar el neumático <strong id="retiro_neumatico_codigo"></strong>. Esta
                            acción registrará el retiro en el historial.</p>
                    </div>
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <div class="input-group">
                                <input type="number" class="form-control" id="horometro_retiro" name="horometro_retiro"
                                    min="0">
                                <span class="input-group-text">hrs</span>
                            </div>
                        </div>
...
// Mostrar información del neumático seleccionado
function mostrarInfoNeumatico() {
    const select = $('#neumatico_id');
    const option = select.find('option:selected');

    if (option.val()) {
        $('#costoNeumatico').text('<?php
...
if ($_POST) {
    try {
        ...
        if ($_POST['action'] === 'rotar') {
            ...
            $db->query("UPDATE instalaciones SET activo = 0 WHERE id IN (?, ?)", 
            ...
            ", [
                $origen['neumatico_id'], $origen['equipo_id'], $destino['posicion'],
                $_POST['fecha_rotacion'], $_POST['horometro_rotacion'],
                $_POST['cocada_origen'], $_POST['motivo_rotacion']
            ]);
            ...
            ", [
                $destino['neumatico_id'], $destino['equipo_id'], $origen['posicion'],
                $_POST['fecha_rotacion'], $_POST['horometro_rotacion'],
                $_POST['cocada_destino'], $_POST['motivo_rotacion']
            ]);
            ...
            ", [
                $origen['neumatico_id'], $origen['equipo_id'], $origen['posicion'],
                $origen['equipo_id'], $destino['posicion'], $_POST['fecha_rotacion'],
                $_POST['horometro_rotacion'], $_POST['motivo_rotacion'], $_POST['cocada_origen']
            ]);
            ...
            ", [
                $destino['neumatico_id'], $destino['equipo_id'], $destino['posicion'],
                $destino['equipo_id'], $origen['posicion'], $_POST['fecha_rotacion'],
                $_POST['horometro_rotacion'], $_POST['motivo_rotacion'], $_POST['cocada_destino']
            ]);
        ...
        if ($_POST['action'] === 'retirar') {
            ...
            $db->query("UPDATE neumaticos SET estado = ? WHERE id = ?", 
            ...
            ", [
                $_POST['neumatico_id'], $instalacion['equipo_id'], $instalacion['posicion'],
                $_POST['fecha_retiro'], $_POST['horometro_retiro'],
                $_POST['motivo_retiro'], $_POST['cocada_retiro']
            ]);
...
?>
                ...
                .sidebar {
                    min - height: 100 vh;
                    background: linear - gradient(135 deg, #2c3e50 0%, # 34495 e 100 % );
                }
                .sidebar.nav - link {
                    color: #ecf0f1;
                ...
                .card {
                    border: none;
                    border - radius: 1 rem;
                    box - shadow: 0 0.125 rem 0.25 rem rgba(0, 0, 0, 0.075);
                }
                .search - box {
                    background: white;
                    border - radius: 1 rem;
                ...
                .stat - card {
                    border: none;
                    border - radius: 1 rem;
                    box - shadow: 0 0.5 rem 1 rem rgba(0, 0, 0, 0.1);
                ...
                href = "#"
                onclick = "verVistaEquipos()" >
                <
                i class = "bi bi-truck" > < /i> Vista por Equipos < /
                a > < /li > <
                li > < a class = "dropdown-item"
                href = "#"
                onclick = "generarReporteRotaciones()" >
                <
                ...
                li > < a class = "dropdown-item"
                href = "#"
                onclick = "planificarMantenimiento()" >
                <
                ...
                h3 class = "mb-0" > <?= formatCurrency($estadisticas['valor_total_instalado']) ?> < /h3> < /
                div > <
                div class = "align-self-center" >
                <
                span class = "h2"
                style = "font-family:inherit; font-weight: bold;" > S / < /span>
                ...
                /td> < /
                tr >
                <?php else: ?> <?php foreach ($instalaciones as $instalacion): ?> <
                tr class = "instalacion-row"
                data - instalacion - id = "<?= $instalacion['id'] ?>" >
                <
                td >
                <
                div class = "posicion-badge bg-<?= 
                                                        $instalacion['posicion'] <= 2 ? + parseFloat(option.data('costo')).toLocaleString());
                $('#remanenteNeumatico').text(option.data('remanente'));
                $('#medidaNeumatico').text(option.data('medida'));
                $('#infoNeumatico').show();
            } else {
                $('#infoNeumatico').hide();
            }
        ...
        function cargarNeumaticosParaRotacion(instalacionOrigenId) {
            $.ajax({
                ...
                method: 'GET',
                data: { instalacion_origen: instalacionOrigenId },
                success: function(response) {
        ...
        $('#instalarForm').on('submit', function(e) {
            ...
            // Show loading state
            $(this).find('button[type="submit"]').html('<i class="bi bi-hourglass-split"></i> Instalando...').prop('disabled', true);
        });
        ...
        $('#rotarForm').on('submit', function(e) {
            ...
            // Show loading state
            $(this).find('button[type="submit"]').html('<i class="bi bi-hourglass-split"></i> Rotando...').prop('disabled', true);
        });
        ...
        $('#retirarForm').on('submit', function(e) {
            ...
            if (destino === 'desecho') {
                if (!confirm('¿Está seguro de que desea enviar este neumático a desecho? Esta acción no se puede deshacer.')) {
                    e.preventDefault();
                    return false;
                }
            }
            ...
            // Show loading state
            $(this).find('button[type="submit"]').html('<i class="bi bi-hourglass-split"></i> Retirando...').prop('disabled', true);
        });
        
        // Reset forms when modals are hidden
        $('.modal').on('hidden.bs.modal', function() {
            $(this).find('form')[0].reset();
            $(this).find('button[type="submit"]').html($(this).find('button[type="submit"]').data('original-text')).prop('disabled', false);
